...
public static class OdinHierarchyDrawer
{
    ...
    static OdinHierarchyDrawer()
    {
        ...
        foreach (PropertyInfo item in typeof(EditorIcons).GetProperties(Flags.StaticPublic).OrderBy(x => x.Name))
        {
    ...
    private static void Draw(int id, Rect rect)
    {
        ...
        if (item != null)
        {
            ...
            if (item.icon && string.IsNullOrEmpty(item.iconName) == false)
            {
                ...
                float rightPadding = ohsStatic.baseIconRightPadding + item.iconRightPaddingAdded;

                EditorGUIUtility.SetIconSize(new Vector2(iconSize, iconSize));
                Rect iconRect = new Rect(rect.xMax - (iconSize + rightPadding), rect.yMin + ((rect.height - iconSize) / 2.0f), iconSize + rightPadding, iconSize);
                //SirenixEditorGUI.DrawSolidRect(iconDrawRect, Color.yellow); //For debugging
                GUIContent iconContent = new GUIContent(editorIcon.Active);
