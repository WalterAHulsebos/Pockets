...
{	
	public static partial class Greasy
	{
		...
		private sealed class GreaserSingleton : EnsuredSingleton<GreaserSingleton>{}
		private static GreaserSingleton Greaser => GreaserSingleton.Instance;
		...
		private static Coroutine CreateInterpolater(float duration, EaseMethod easeMethod, ProgressChanged onProgress)
			=> Greaser.StartCoroutine(Lerper(duration, easeMethod, onProgress));
