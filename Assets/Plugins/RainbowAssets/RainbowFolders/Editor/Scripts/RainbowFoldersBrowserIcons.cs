    ...
    */
    ...
    public class RainbowFoldersBrowserIcons
    {
        ...
        private static void DrawEditIcon(string guid, Rect rect)
        {
            ...
            Texture2D editIcon = RainbowFoldersEditorUtility.GetEditFolderIcon(isSmall, EditorGUIUtility.isProSkin);
        ...
        private static void InitCollabDelegates(Assembly assembly)
        {
            try
            {
                Type collabAccessType = assembly.GetType("UnityEditor.Web.CollabAccess");
                object collabAccessInstance = collabAccessType.GetProperty("Instance", BindingFlags.Static | BindingFlags.Public).GetValue(null, null);
                MethodInfo collabAccessMethod = collabAccessInstance.GetType().GetMethod("IsServiceEnabled", BindingFlags.Instance | BindingFlags.Public);
                _isCollabEnabled = (Func<bool>) Delegate.CreateDelegate(typeof(Func<bool>), collabAccessInstance, collabAccessMethod);

                Type collabHookType = assembly.GetType("UnityEditor.Collaboration.CollabProjectHook");

                #if UNITY_2017_1_OR_NEWER
                    MethodInfo collabHook = collabHookType.GetMethod("OnProjectWindowIconOverlay", BindingFlags.Static | BindingFlags.Public);
        ...
        private static void DrawCustomIcon(string guid, Rect rect, Texture texture, bool isSmall)
        {
            ...
            if (iconRect.width > LARGE_ICON_SIZE)
            {
                ...
                float offset = (iconRect.width - LARGE_ICON_SIZE) / 2f;
            ...
            if (_isCollabEnabled())
            {
                Texture2D background = RainbowFoldersEditorUtility.GetCollabBackground(isSmall, EditorGUIUtility.isProSkin);
